...
def sample_noniid_data(args):
    ...
    sk_train_x, sk_test_x, sk_train_y, sk_test_y = train_test_split(x[sk_idx], y[sk_idx], train_size=target_size, test_size=int(gamma*target_size))
...
def load_data_new(data_name, args):
    np.random.seed(42)
    ...
    # Consistent D_ and pool
    fixed_size = args.target_data_size - 1000
    pool_size = 1000

    D_x = train_x[:fixed_size]
    D_y = train_y[:fixed_size]

    # records to sample from
    pool_x = train_x[fixed_size:fixed_size + pool_size]
    pool_y = train_y[fixed_size:fixed_size + pool_size]

    test_x = test_x[:int(args.target_test_train_ratio * args.target_data_size)]
    test_y = test_y[:int(args.target_test_train_ratio * args.target_data_size)]

    return D_x, D_y, pool_x, pool_y, test_x, test_y
...
def threat_model(args, tot_len):
    ...
    adv_known_idxs['high'] = list(range(train_size))
