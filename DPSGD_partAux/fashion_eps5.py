...
for run_idx in range(runs):
  ...
  for target_idx in range(0, config.num_in_prior):
    aux_label_new = prior_labels[target_idx]
    aux_set_new = aux_dict[aux_label_new]
    predicted_index = reco_attack_u_rero(info_for_attacker, target_idx, aux_set_new)
...
with open(f'./DPSGD_partAux/results/fashion_raw/fashion_eps{eps}_idx{correct_idx}_partAux_big.csv', mode='w', newline='') as file:
