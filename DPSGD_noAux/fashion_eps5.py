...
config.noise_multiplier = 9.5  # @param
config.l2_norm_clip = 1  # @param
config.epochs = 100  # @param
config.learning_rate = 0.5  # @param
...
def load_dataset(
  ...
  """Loads the Fashion MNIST dataset as a generator of batches."""
  ds = tfds.load(
      'fashion_mnist', split=split + f'[{start_idx}:{start_idx+total_num}]'
  ).cache()
...
# Generate the fixed set if config.total_num > 1.
if config.total_num > 1:
  ...
  for curr_train_batch in train_data:
    imgs = curr_train_batch['image'].astype(np.float32) / 255.0
    fixed_images.extend(imgs)
...
prior_images = prior_batch['image'].astype(np.float32) / 255.0
prior_labels = prior_batch['label']


# Load Fashion MNIST test set
ds = tfds.load('fashion_mnist', split='test', batch_size=-1, as_supervised=True)
...
def loss(params, batch):
  ...
  inputs = jnp.reshape(inputs, (-1, 28, 28, 1))  # Ensure correct shape
...
def train_fn(attack_config, config):
  ...
  params = net.init(rng, np.ones((1, 28, 28, 1), dtype=np.float32))
...
def reconstruction_attack(config, attack_config):
  ...
  return improved_attack_cand_idx, accuracy
...
for run_idx in range(runs):
  ...
  improved_attack_cand_idx, accuracy = reconstruction_attack(config, attack_config)
  predicted_indeces.append(improved_attack_cand_idx)
  test_accuracies.append(accuracy)

# Compute ReRo and U-ReRo
counter = Counter(predicted_indeces)
probs = {i: (counter.get(i, 0) / runs) for i in range(config.num_in_prior)}

rero = probs[correct_idx] * (1 / config.num_in_prior)

correction = 0
for idx in range(config.num_in_prior):
  correction += probs[idx] * (1 / config.num_in_prior**2)

u_rero = rero - correction
...
with open(f'./DPSGD_noAux/results/fashion_raw/fashion_eps{eps}_idx{correct_idx}_noAux.csv', mode='w', newline='') as file:
  writer = csv.writer(file)
  writer.writerow(["Eps", 'ReRo', 'U-ReRo','Hayes bound', 'Balle bound', 'Test acc'])
  writer.writerow([eps, rero, u_rero, rub, balle_bound, test_accuracy]) 
