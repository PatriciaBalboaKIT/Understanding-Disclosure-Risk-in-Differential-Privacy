...
raw_results_folder = main_dir / "DPSGD_fullAux_nonUnif" / "results" / "mnist_raw"

# Data distribution: most distinguishable images are at index 3,4
...
for eps in epsilons:
    output_file = main_dir / "DPSGD_fullAux_nonUnif" / "results" / f"mnist_eps{eps}.csv"
    dfs = []
    ...
    weighted_sum = sum(prior[i] * dfs[i][["ReRo", "U-ReRo"]] for i in range(candidate_set_size))
    averages[["ReRo", "U-ReRo"]] = weighted_sum.sum(axis=0)
