...
results_fashion = []
for file in results_dir.glob('mnist_eps*.csv'):
    df = pd.read_csv(file)
...
plt.scatter(results['Eps'], results['ReRo'], label='ReRo', marker='o',color="C0", s=50)
plt.scatter(results['Eps'], results['U-ReRo'], label='RAD', marker='x',color="C1", s=80, zorder=10)
plt.plot(results['Eps'], aux_bound, label="RAD bound (Th.4.2)", lw=3, color="#2ca02c")
plt.plot(results['Eps'], results['Hayes bound'], lw=3, label="Hayes et al.", color="C0")
...
plt.tight_layout()
plt.savefig(fig_dir / "DPSGD_fullAux_nonUnif_MNIST.png")