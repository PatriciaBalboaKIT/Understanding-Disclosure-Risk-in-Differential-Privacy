...
config.noise_multiplier = 9.5  # @param, change this for different eps
...
######################## ATTACK ###################################
def reconstruction_attack(config, attack_config, target_idx, run_idx):
  ...
  # we guess that improved_dot_prod_cands holds P(theta|y) for all y
  # x: correct_idx
  # compute w(theta,x) = Pr(theta|x) - 1/m * sum(Pr(theta|y) for all y)
  if improved_dot_prod_cands[target_idx] > np.mean(improved_dot_prod_cands):
    return target_idx, info_for_attacker, accuracy
  else:
    other_indexes = [idx for idx in range(config.num_in_prior) if idx != target_idx]
    return np.random.choice(other_indexes), info_for_attacker, accuracy
  

def reco_attack_u_rero(info_for_attacker, target_idx):
  (
      params_over_time,
      private_grad_over_time,
      maybe_noisy_target_grad_over_time,
...
# skewed data distribution: most distinguishable images are at index 3,4
weights = np.array([0.01, 0.02, 0.07, 0.35, 0.35, 0.1, 0.05, 0.05])
...
for run_idx in range(runs):
  ...
  improved_attack_cand_idx, info_for_attacker, accuracy = reconstruction_attack(config, attack_config, correct_idx, run_idx)
  ...
  for target_idx in range(0, config.num_in_prior):
    predicted_index = reco_attack_u_rero(info_for_attacker, target_idx)
    model_results += (predicted_index == target_idx) * prior[target_idx]
  round_correction.append(model_results)
...
rub = reconstruction_upper_bound(prior[correct_idx], config.q, config.noise_multiplier, config.steps)
balle_bound = np.exp(eps) * prior[correct_idx]
print(f"\nHayes bound: {rub:.3f}, ReRo: {rero:.3f}, U-ReRo: {u_rero:.3f},  Balle bound: {balle_bound:.3f}, Test accuracy: {test_accuracy}")

with open(f'./DPSGD_fullAux_nonUnif/results/mnist_raw/mnist_eps{eps}_idx{correct_idx}_fullAux_big.csv', mode='w', newline='') as file:
